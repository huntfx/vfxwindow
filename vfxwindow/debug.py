...
class TestWindow(VFXWindow):
    ...
    def __init__(self, **kwargs):
        ...
        if self.application == 'Maya':
            ...
            self.callbacks.add('render.start', persistent(lambda scene, _: print('Callback: render.start ({})'.format(scene.name))))
            self.callbacks.add('render.end', persistent(lambda scene, _: print('Callback: render.end ({})'.format(scene.name))))
            self.callbacks.add('render.frame.start', persistent(lambda scene, _: print('Callback: render.frame.start ({})'.format(scene.name))))
            self.callbacks.add('render.frame.end', persistent(lambda scene, _: print('Callback: render.frame.end ({})'.format(scene.name))))
            ...
            self.callbacks.add('playback.end', persistent(lambda scene, depsgraph: print('Callback: playback.end ({})'.format(scene.name))))
            ...
            self.callbacks.add('ui.explorer.selection.changed', lambda explorerID, uiMgr, action, originalExplorerID: print('explorer.selection.changed (explorerID={!r}, uiMgr={!r}, action={!r}, originalExplorerID={!r})'.format(explorerID, uiMgr, action, originalExplorerID)))
            ...
            self.callbacks.add('export.textures.start', lambda evt: print('export.textures.start (textures={!r})'.format(evt.textures)))
            self.callbacks.add('export.textures.end', lambda evt: print('export.textures.end (message={!r}, status={!r}, textures={!r})'.format(evt.message, evt.status, evt.textures)))
